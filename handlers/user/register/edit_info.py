...
from keyboards.default.user.edit_info import edit_text, edit_register_info_kb
from keyboards.default.user.register import without_flat_kb, phone_share_kb, without_flat_text
...
from states.user import EditRegisterState
...
async def handle_buttons(message: types.Message, state: FSMContext):
    ...
    if button_type == edit_text['first_name_text']:
        ...
        return await message.answer("Впишіть будь-ласка ваше ім'я")
    ...
    if button_type == edit_text['house_text']:
        ...
        return await message.answer("Впишіть будь ласка номер будинку 🏠")
...
async def send_user_info(state: FSMContext, **kwargs):
    ...
    info_text = (
        # TODO Email
        # f"Пошта: {user_data['']}"
        f"Ім'я: {user_data['FirstName']}\n"
        f"Прізвище: {user_data['MiddleName']}\n"
        f"По батькові: {user_data['LastName']}\n"
        f"Телефон: {user_data['Phone']}\n"
        f"Пароль: {user_data['Password']}\n"
        f"Вулиця: {user_data['StreetId']}\n"
        f"Номер будинку: {user_data['House']}\n\n"
        "Підтверджуйте 👇"
    )
